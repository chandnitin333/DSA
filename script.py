...
students_collection = mongo_db['icad_students']
topics_collection = mongo_db['topics']
subtopics_collection = mongo_db['icad_subtopics']
concepts_collection = mongo_db['icad_concepts']
sub_concepts_collection = mongo_db['icad_sub_concepts']
subjects_collections = mongo_db['icad_subjects']
subjects_allocation_collections = mongo_db['icad_course_subject_allocations']
...
def migrate_topics(cursor):
    ...
    for row in cursor.fetchall():
        ...
        result = topics_collection.insert_one(topic_document)
        topic_id_mapping[row["TOPIC_ID"]] = result.inserted_id
...
def migrate_subtopics(cursor):
    ...
    for row in cursor.fetchall():
        ...
        subtopic_document = {
            ...
            "topic_id": topic_id_mapping.get(row["TOPIC_ID"]),
...
def migrate_subjects(cursor):
    """Migrate concepts from MySQL to MongoDB."""
    cursor.execute("SELECT * FROM icad_subject_mst")
    for row in cursor.fetchall():
        ...
        subject_document = {
            ...
            "remarks": row["REMARKS"],
            "created_at": handle_datetime(row["CREATED_ON"]),
            "updated_at": handle_datetime(row["LAST_MODIFIED"]),
        }
        try:
            result = subjects_collections.insert_one(subject_document)
            
        except Exception as e:
            print(f"Failed to insert concept: {row['SUBJECT_ID']} - Error: {e}")

def migrate_subjects_allocation(cursor):
    """Migrate concepts from MySQL to MongoDB."""
    course_id_mapping = {}
    courses_collection
    for document in courses_collection.find(): 
        course_id_mapping[document["course_code"]] = document["_id"] 
    
    cursor.execute("SELECT * FROM icad_course_subject_allocation")
    for row in cursor.fetchall():
        course_id = course_id_mapping.get(row["COURSE_ID"])
        # print("COURSE ID===", course_id)
        # print(row)
        subject_alloc_document = {
            'subject_allocation_id': row["COURSE_SUBJECT_ALLOCATION_ID"],
            'course_id': course_id,
            "subject_id": row["SUBJECT_ID"],
            "sequence": row["SEQUENCE"],
            "is_deleted": row["IS_DELETED"],
        ...
        try:
            result = subjects_allocation_collections.insert_one(subject_alloc_document)
...
def hash_password(password):
    ...
    try:
        # Use gensalt with 12 rounds to match Node.js
...
try:
    with mysql_conn.cursor(pymysql.cursors.DictCursor) as cursor:
        ...
        migrate_subjects_allocation(cursor)
finally:
    mysql_conn.close()
    mongo_client.close()
